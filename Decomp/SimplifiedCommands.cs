...
public class ExtractNewCommand : Command
{
    public ExtractNewCommand() : base("extract", "Extract modules from webpack bundle")
    {
        ...
        var autoDeobfuscateOpt = new Option<bool>("--auto", () => true, "Auto-apply known mappings");
        ...
        {
            var extractor = new SimplifiedExtractor();
            var result = await extractor.ExtractAsync(bundle, version, autoDeobfuscate);
...
public class SmartRenameCommand : Command
{
    ...
    {
        var fileArg = new Argument<string>("file", "File to rename (auto-detects version)");
        ...
        this.SetHandler(async (file) =>
        {
            ...
            var (filePath, version) = FindFile(file);
            if (filePath == null)
            {
            ...
            var response = await LspClient.RenameAsync(filePath, mappings);
    ...
    private static (string? path, string? version) FindFile(string file)
    {
        ...
        if (File.Exists(file))
        {
            ...
            var match = Regex.Match(file, @"decomp[/\\](\d+\.\d+\.\d+)(-dev)?[/\\]");
            return (file, match.Success ? match.Groups[1].Value : "unknown");
        }
        
        // Search in decomp directory for the file
        var decompDir = "decomp";
        if (!Directory.Exists(decompDir)) return (null, null);
        
        // Look in all version-dev directories first (most likely place)
        foreach (var dir in Directory.GetDirectories(decompDir, "*-dev"))
        {
            var testPath = Path.Combine(dir, file);
            if (File.Exists(testPath))
            {
                var version = Path.GetFileName(dir).Replace("-dev", "");
                return (testPath, version);
            }
        }
        
        // Then check original version directories
        foreach (var dir in Directory.GetDirectories(decompDir).Where(d => !d.Contains("-")))
        {
            var testPath = Path.Combine(dir, file);
            if (File.Exists(testPath))
            {
...
public class RenameAllCommand : Command
{
    ...
    {
        var versionOpt = new Option<string?>("--version", "Specific version (default: latest)");
        ...
        this.SetHandler(async (version) =>
        {
            ...
            var targetVersion = version ?? GetLatestDevVersion();
            ...
            var devDir = Path.Combine("decomp", $"{targetVersion}-dev");
            ...
            var files = Directory.GetFiles(devDir, "*.js");
            ...
            foreach (var file in files)
            {
                try
                {
                    var response = await LspClient.RenameAsync(file, mappings);
...
public class FinalizeCommand : Command
{
    ...
    {
        var versionOpt = new Option<string?>("--version", "Version to finalize (default: latest)");
        
        AddOption(versionOpt);
        
        this.SetHandler(async (version) =>
        {
            var targetVersion = version ?? GetLatestVersion();
            if (targetVersion == null)
            {
...
public class CleanupCommand : Command
{
    ...
    {
        var keepOpt = new Option<int>("--keep", () => 1, "Number of recent versions to keep");
        var dryRunOpt = new Option<bool>("--dry-run", "Show what would be deleted without deleting");
        ...
        this.SetHandler((keep, dryRun) =>
        {
            ...
            foreach (var dir in toDelete)
            {
                if (dryRun)
                {
                    ...
                    var finalDir = dir.Path.Replace("-dev", "-final");
...
public class ListVersionsCommand : Command
{
    public ListVersionsCommand() : base("versions", "List all extracted versions")
    {
        this.SetHandler(() =>
        {
            ...
            foreach (var dir in Directory.GetDirectories("decomp"))
            {
                var name = Path.GetFileName(dir);
                if (name.EndsWith("-dev"))
                {
                    var version = name.Replace("-dev", "");
                ...
                {
                    var version = name.Replace("-final", "");
                    var info = versions.FirstOrDefault(v => v.Version == version);
                    if (info == null)
                    {
                        info = new VersionInfo { Version = version };
                ...
                {
                    var info = versions.FirstOrDefault(v => v.Version == name);
                    if (info == null)
                    {
            ...
            {
                var json = File.ReadAllText("decomp/mappings.json");
                var mappings = JsonSerializer.Deserialize<Dictionary<string, string>>(json);
                Console.WriteLine();

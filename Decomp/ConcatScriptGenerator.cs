...
";
		...
		public static async Task GenerateScriptAsync(string agentName, string agentDir) {
			using IDisposable timer = Logger.TimeOperation($"Generate concat script for {agentName}");

			string scriptPath = Path.Combine(agentDir, "concat-codebase.sh");
...
if [ -d ""$PACKAGE_DIR/vendor"" ]; then
    ...
    find ""$PACKAGE_DIR/vendor"" -type f \( -name ""*.exe"" -o -perm -111 \) | while read -r binary; do
		...
		public static async Task GenerateMasterScriptAsync(string decompDir) {
			using IDisposable timer = Logger.TimeOperation("Generate master concat script");

			string scriptPath = Path.Combine(decompDir, "concat-all.sh");

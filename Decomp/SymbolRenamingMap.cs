...
namespace Hoho.Decomp {
	...
	public class SymbolRenamingMap {
		private readonly Dictionary<string, string>                     _globalMappings = new Dictionary<string, string>();
		private readonly Dictionary<string, Dictionary<string, string>> _scopedMappings = new Dictionary<string, Dictionary<string, string>>();
		...
		public int Count {
			get {
				...
				foreach (Dictionary<string, string> scopeMappings in _scopedMappings.Values) {
		...
		public string? GetMapping(string originalName, string scope) {
			...
			for (int i = parts.Length - 1; i > 0; i--) {
				string parentScope = string.Join(".", parts[..i]);
		...
		public Dictionary<string, string> GetScopeMappings(string scope) =>
			_scopedMappings.ContainsKey(scope)
		...
		/// </summary>
		public Dictionary<string, string> GetGlobalMappings() => new Dictionary<string, string>(_globalMappings);
		...
		/// </summary>
		public string? GetMapping(string originalName) => _globalMappings.ContainsKey(originalName) ? _globalMappings[originalName] : null;
		...
		public static SymbolRenamingMap FromJson(string json) {
			SymbolRenamingMap map  = new SymbolRenamingMap();
			JsonElement       data = JsonSerializer.Deserialize<JsonElement>(json);

			if (data.TryGetProperty("GlobalMappings", out JsonElement globalMappings)) {
				foreach (JsonProperty kvp in globalMappings.EnumerateObject()) {
					map.AddGlobalMapping(kvp.Name, kvp.Value.GetString() ?? "");
				}
			}

			if (data.TryGetProperty("ScopedMappings", out JsonElement scopedMappings)) {
				foreach (JsonProperty scope in scopedMappings.EnumerateObject()) {
					foreach (JsonProperty mapping in scope.Value.EnumerateObject()) {

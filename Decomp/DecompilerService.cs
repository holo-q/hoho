...
namespace Hoho.Decomp {
	...
	public static class DecompilerService {
		...
		public static async Task SetupAsync() {
			using IDisposable timer = Logger.TimeOperation("Decomp Setup");
			...
			string[] agents = new[] { "claude-code" }; // Start with Claude Code
			Logger.Info("Analyzing {AgentCount} CLI agents", agents.Length);

			IEnumerable<Task> tasks = agents.Select(AnalyzeAgentAsync);
			await Task.WhenAll(tasks);

			// Generate concatenation scripts
			Console.WriteLine("\nGenerating concatenation scripts...");
			foreach (string agent in agents) {
				string agentDir = Path.Combine("decomp", agent);
				if (Directory.Exists(agentDir)) {
					...
					string symbolMap = await TreeSitterIndexer.GenerateIndexAsync(agentDir);

					if (!string.IsNullOrEmpty(symbolMap)) {
						string mapPath = Path.Combine(agentDir, "symbol-map.md");
					...
					string cliPath = Path.Combine(agentDir, "package", "cli.js");
					if (File.Exists(cliPath)) {
						Console.WriteLine("Analyzing webpack bundle structure...");
						WebpackBundleAnalyzer.BundleAnalysis bundleAnalysis = await WebpackBundleAnalyzer.AnalyzeBundleAsync(cliPath);
						string                               report         = bundleAnalysis.GenerateReport();

						string reportPath = Path.Combine(agentDir, "bundle-analysis.txt");
		...
		public static async Task AnalyzeAgentAsync(string agentName) {
			...
			string agentDir = Path.Combine("decomp", agentName);
		...
		private static async Task AnalyzeClaudeCodeAsync(string outputDir) {
			...
			HttpResponseMessage response = await _httpClient.GetAsync(npmRegistry);
			response.EnsureSuccessStatusCode();
			string          jsonString  = await response.Content.ReadAsStringAsync();
			NpmPackageInfo? packageInfo = JsonSerializer.Deserialize(jsonString, JsonContext.Default.NpmPackageInfo);
			if (packageInfo?.DistTags?.Latest == null) {
				Console.WriteLine("Could not fetch Claude Code package info");
				return;
			}

			string? version = packageInfo.DistTags.Latest;
			Console.WriteLine($"Found @anthropic-ai/claude-code v{version}");

			// Download tarball
			string tarballUrl  = $"https://registry.npmjs.org/@anthropic-ai/claude-code/-/claude-code-{version}.tgz";
			string tarballPath = Path.Combine(outputDir, $"claude-code-{version}.tgz");
			...
			string formattedDir = Path.Combine(outputDir, "formatted");
		...
		private static async Task ExtractTarGzAsync(string tarGzPath, string extractPath) {
			...
			string outputFile = Path.Combine(extractPath, "package-contents.bin");

			await using FileStream outputStream = File.Create(outputFile);
		...
		private static async Task AnalyzeExtractedPackageAsync(string extractDir, string outputDir) {
			string analysisDir = Path.Combine(outputDir, "analysis");
			Directory.CreateDirectory(analysisDir);

			// Create file structure analysis
			string                   fileListPath = Path.Combine(analysisDir, "file_structure.txt");
			await using StreamWriter writer       = File.CreateText(fileListPath);
		...
		private static void EnsureDirectories() {
			string[] dirs = new[] { "decomp", "decomp/claude-code", ".cache", ".tmp" };

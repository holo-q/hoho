...
public class VersionManager
{
    ...
    public async Task<string> SetupVersionAsync(string version, string? sourcePath = null)
    {
        ...
        var automatedDir = Path.Combine(versionDir, "automated");
        var analysisDir = Path.Combine(versionDir, "analysis");
        ...
        else if (File.Exists(sourcePath))
        {
            ...
            var destPath = Path.Combine(originalDir, Path.GetFileName(sourcePath));
    ...
    public async Task<LearningResult> LearnFromDirectoryAsync(string version)
    {
        ...
        var result = new LearningResult { Version = version };
        
        // Process main files
        var mainFiles = new[] { "cli.js", "sdk.mjs", "sdk.d.ts" };
        foreach (var file in mainFiles)
        {
            var origPath = Path.Combine(originalDir, file);
            var manualPath = Path.Combine(manualDir, file);
            
            if (File.Exists(origPath) && File.Exists(manualPath))
            {
                Logger.Info($"Learning from {file}...");
                var mappings = await _mapper.LearnMappings(origPath, manualPath);
        ...
        var origModulesDir = Path.Combine(originalDir, "modules");
        var manualModulesDir = Path.Combine(manualDir, "modules");
        ...
        if (Directory.Exists(origModulesDir) && Directory.Exists(manualModulesDir))
        {
            ...
            var matches = await MatchModuleFilesAsync(origModulesDir, manualModulesDir);
            
            foreach (var (origFile, manualFile) in matches)
            {
                Logger.Info($"Learning from module {Path.GetFileName(origFile)} -> {Path.GetFileName(manualFile)}");
                var mappings = await _mapper.LearnMappings(origFile, manualFile);
                
                var moduleKey = $"modules/{Path.GetFileName(origFile)}";
                result.FileMappings[moduleKey] = mappings;
                result.TotalMappings += mappings.TotalMappings;
                
                // Learn file renaming pattern
                var origName = Path.GetFileNameWithoutExtension(origFile);
                var manualName = Path.GetFileNameWithoutExtension(manualFile);
                result.FileRenames[origName] = manualName;
            }
        }
        
        // Save version-specific mappings
        var versionMappingPath = Path.Combine(_baseDir, "mappings", "version-mappings", $"{version}.json");
    ...
    {
        var targetDir = Path.Combine(_baseDir, "versions", targetVersion);
        var originalDir = Path.Combine(targetDir, "original");
        var automatedDir = Path.Combine(targetDir, "automated");
        ...
        // Apply to main files
        var mainFiles = Directory.GetFiles(originalDir, "*.*", SearchOption.TopDirectoryOnly);
        foreach (var origFile in mainFiles)
        {
            var fileName = Path.GetFileName(origFile);
            var outputPath = Path.Combine(automatedDir, fileName);
            
            Logger.Info($"Processing {fileName}...");
            var deobfuscated = await _mapper.ApplyMappings(origFile);
            await File.WriteAllTextAsync(outputPath, deobfuscated);
        }
        
        // Apply to modules
        var modulesDir = Path.Combine(originalDir, "modules");
        if (Directory.Exists(modulesDir))
        {
            var outputModulesDir = Path.Combine(automatedDir, "modules");
            Directory.CreateDirectory(outputModulesDir);
            
            // Load file renames from previous version
            var fileRenames = await LoadFileRenamesAsync(sourceVersion);
            
            foreach (var moduleFile in Directory.GetFiles(modulesDir))
            {
                var origName = Path.GetFileNameWithoutExtension(moduleFile);
                var newName = fileRenames.TryGetValue(origName, out var renamed) ? renamed : origName;
                var outputPath = Path.Combine(outputModulesDir, $"{newName}.js");
                
                Logger.Info($"Processing module {origName} -> {newName}...");
                var deobfuscated = await _mapper.ApplyMappings(moduleFile);
                await File.WriteAllTextAsync(outputPath, deobfuscated);
            }
        }
        
        // Generate comparison report
        var report = await GenerateComparisonReportAsync(targetVersion, originalDir, automatedDir);
        var reportPath = Path.Combine(targetDir, "analysis", "automated-deobfuscation-report.md");
    ...
    private async Task ExtractModulesFromBundleAsync(string bundlePath, string outputDir)
    {
        ...
        foreach (Match match in matches)
        {
            ...
            var moduleContent = match.Value;
            
            var modulePath = Path.Combine(outputDir, $"{moduleName}.js");
        ...
        foreach (Match match in matches)
        {
            ...
            var classContent = match.Value;
            
            var classPath = Path.Combine(outputDir, $"{className}.js");
    ...
    private async Task CreateManualTemplateAsync(string originalDir, string manualDir)
    {
        ...
        var readmeContent = @"# Manual Decompilation Directory
...
";
        ...
        foreach (var file in Directory.GetFiles(originalDir, "*.*", SearchOption.TopDirectoryOnly))
        {
            ...
            var placeholder = $"// Manual deobfuscation of {fileName}\n// Original: ../original/{fileName}\n\n";
    ...
    private async Task<List<(string original, string manual)>> MatchModuleFilesAsync(string origDir, string manualDir)
    {
        ...
        var origFiles = Directory.GetFiles(origDir, "*.js");
        var manualFiles = Directory.GetFiles(manualDir, "*.js");
        
        // Use structural matching to pair files
        foreach (var origFile in origFiles)
        {
            var origContent = await File.ReadAllTextAsync(origFile);
            double bestScore = 0;
            ...
            foreach (var manualFile in manualFiles)
            {
                ...
                var manualContent = await File.ReadAllTextAsync(manualFile);
                var score = CalculateStructuralSimilarity(origContent, manualContent);
    ...
    private double CalculateStructuralSimilarity(string orig, string edited)
    {
        ...
        var origStructure = ExtractStructure(orig);
        var editedStructure = ExtractStructure(edited);
        
        // Compare control flow, string literals, etc.
        var origFlow = Regex.Matches(orig, @"\b(if|else|for|while|switch|return|throw|try|catch)\b").Count;
        var editedFlow = Regex.Matches(edited, @"\b(if|else|for|while|switch|return|throw|try|catch)\b").Count;
        
        var flowSimilarity = 1.0 - Math.Abs(origFlow - editedFlow) / (double)Math.Max(Math.Max(origFlow, editedFlow), 1);
        
        // Compare string literals
        var origStrings = ExtractStringLiterals(orig);
        var editedStrings = ExtractStringLiterals(edited);
        
        var commonStrings = origStrings.Intersect(editedStrings).Count();
        var stringSimilarity = origStrings.Count > 0 
    ...
    private HashSet<string> ExtractStringLiterals(string code)
    {
        ...
        var matches = Regex.Matches(code, @"[""']([^""']{3,})[""']");
        foreach (Match match in matches)
        {
    ...
    {
        var mappingPath = Path.Combine(_baseDir, "mappings", "version-mappings", $"{version}.json");
        
        if (!File.Exists(mappingPath))
            return new Dictionary<string, string>();
            
        var json = await File.ReadAllTextAsync(mappingPath);
        var result = JsonSerializer.Deserialize<LearningResult>(json);
    ...
    private async Task<string> GenerateComparisonReportAsync(string version, string originalDir, string automatedDir)
    {
        ...
        var files = Directory.GetFiles(automatedDir, "*.*", SearchOption.AllDirectories);
        ...
        {
            var fileName = Path.GetFileName(file);
            var origPath = Path.Combine(originalDir, Path.GetRelativePath(automatedDir, file));
            
            if (File.Exists(origPath))
            {
                var origContent = await File.ReadAllTextAsync(origPath);
                var newContent = await File.ReadAllTextAsync(file);
                
                // Show first function transformation
                var origFunc = Regex.Match(origContent, @"function\s+(\w+)");
                var newFunc = Regex.Match(newContent, @"function\s+(\w+)");
    ...
    {
        var cachePath = Path.Combine(_baseDir, "downloads", $"claude-code-{version}.tgz");
    ...
    {
        var registryPath = Path.Combine(_baseDir, "registry.json");
        ...
        try
        {
            ...
            var registry = JsonSerializer.Deserialize<Dictionary<string, VersionInfo>>(json);
    ...
    private async Task SaveVersionRegistry()
    {
        ...
        var json = JsonSerializer.Serialize(_versions, new JsonSerializerOptions 
        { 
            WriteIndented = true 
        });
    ...
    public void ListVersions()
    {
        ...
        foreach (var version in _versions.OrderByDescending(v => v.Value.DateAdded))
        {
            var info = version.Value;
            Console.WriteLine($"\n{info.Version}:");
...
public class LearningResult
{
    ...
    public Dictionary<string, MappingResult> FileMappings { get; set; } = new();
    public Dictionary<string, string> FileRenames { get; set; } = new();
}
	...
	public class LspServerDaemon {
		...
		public async Task StartAsync() {
			...
			if (File.Exists(LOCK_FILE)) {
				try {
					string pidStr = await File.ReadAllTextAsync(LOCK_FILE);
		...
		private async Task HandleClient(TcpClient client) {
			using (client) {
				try {
					...
					RenameRequest? request = JsonSerializer.Deserialize<RenameRequest>(requestJson);
					if (request == null) {
					...
					// Process rename
					RenameResponse response = await ProcessRenameAsync(request);
		...
		private async Task<RenameResponse> ProcessRenameAsync(RenameRequest request) {
			try {
				...
				// Perform renames
				RenameReport report = await _lspService.BatchRenameAsync(request.FilePath, request.Mappings);
		...
		public static bool IsRunning() {
			...
			try {
				string pidStr = File.ReadAllText(LOCK_FILE);
				if (int.TryParse(pidStr, out int pid)) {
					Process? process = Process.GetProcessById(pid);
	...
	public class LspClient {
		...
		public static async Task<RenameResponse> RenameAsync(string filePath, Dictionary<string, string> mappings) {
			...
			try {
				...
				RenameResponse? response = JsonSerializer.Deserialize<RenameResponse>(responseJson);

...
namespace Hoho.Decomp {
	...
	public static class TreeSitterIndexer {
		...
		public static async Task<string> GenerateIndexAsync(string agentDir) {
			using IDisposable timer = Logger.TimeOperation("Generate tree-sitter index");

			string packageDir = Path.Combine(agentDir, "package");
			...
			string[] dtsFiles = Directory.GetFiles(packageDir, "*.d.ts");
			foreach (string file in dtsFiles) {
				await IndexTypeScriptFileAsync(file, indexBuilder);
			}

			// Process JavaScript modules
			IEnumerable<string> jsFiles = Directory.GetFiles(packageDir, "*.mjs")
		...
		private static async Task IndexTypeScriptFileAsync(string filePath, StringBuilder output) {
			...
			try {
				...
				List<SymbolInfo> symbols = await ExtractSymbolsWithTreeSitterAsync(filePath, "typescript");

				if (symbols.Any()) {
					// Group symbols by type
					IOrderedEnumerable<IGrouping<string, SymbolInfo>> grouped = symbols.GroupBy(s => s.Type)
						.OrderBy(g => GetTypeOrder(g.Key));

					foreach (IGrouping<string, SymbolInfo> group in grouped) {
						List<string> symbolNames = group.OrderBy(s => s.Name).Select(s => s.Name).ToList();
						...
						for (int i = 0; i < symbolNames.Count; i++) {
							...
							string separator = i < symbolNames.Count - 1 ? ", " : "";
		...
		private static async Task IndexJavaScriptFileAsync(string filePath, StringBuilder output) {
			...
			List<SymbolInfo> symbols = await ExtractSymbolsWithTreeSitterAsync(filePath, "javascript");

			// If tree-sitter fails or finds nothing, try minified extraction
			if (!symbols.Any() && fileSize > 100_000) {
				List<string> minifiedSymbols = await ExtractMinifiedSymbolsAsync(filePath);
				...
				if (minifiedSymbols.Any()) {
					...
					for (int i = 0; i < minifiedSymbols.Count; i++) {
						string symbol    = minifiedSymbols[i];
						string separator = i < minifiedSymbols.Count - 1 ? ", " : "";
				...
				foreach (IGrouping<string, SymbolInfo> group in grouped) {
					List<string> symbolList = group.OrderBy(s => s.Name).Select(s => s.Name).ToList();
					output.Append($"  {group.Key}s({group.Count()}): ");
					...
					for (int i = 0; i < symbolList.Count; i++) {
						...
						string separator = i < symbolList.Count - 1 ? ", " : "";
				...
				if ((fileName == "cli.js" || fileName == "sdk.mjs") && symbols.Any(s => s.Type == "Function")) {
					List<string> keyFunctions = symbols
						.Where(s => s.Type == "Function" && !s.Name.StartsWith("_") && s.Name.Length > 4 && !Regex.IsMatch(s.Name, @"^[A-Z][0-9]+"))
						.OrderBy(s => s.Name)
						.Select(s => s.Name)
						.ToList();
					...
					if (keyFunctions.Any()) {
						...
						for (int i = 0; i < keyFunctions.Count; i++) {
							string func      = keyFunctions[i];
							string separator = i < keyFunctions.Count - 1 ? ", " : "";
		...
		private static async Task<List<SymbolInfo>> ExtractSymbolsWithTreeSitterAsync(string filePath, string language) {
			...
			try {
				...
				Process treeSitterCheck = new Process {
		...
		private static async Task<List<SymbolInfo>> ExtractSymbolsWithRegexAsync(string filePath) {
			...
			foreach ((string pattern, string type) in patterns) {
		...
		private static async Task<List<string>> ExtractMinifiedSymbolsAsync(string filePath) {
			...
			foreach (string pattern in patterns) {
				...
				foreach (Match match in matches) {
					if (match.Groups.Count > 1) {
						IEnumerable<string> names = match.Groups[1].Value.Split(',')
							.Select(s => s.Trim())
		...
		private static List<SymbolInfo> ParseTreeSitterOutput(string output) {
			...
			foreach (string line in lines) {
				...
				if (line.Contains("function_declaration") ||
					...
					Match match = Regex.Match(line, @"name:\s*(\w+)");
					if (match.Success) {
						string type = line.Contains("function") ? "Function" :
		...
		private static int CountLines(string filePath) => File.ReadLines(filePath).Count();
		...
		private static string GetFileAnalysis(string fileName, List<SymbolInfo> symbols, long fileSize) {
			int functionCount = symbols.Count(s => s.Type == "Function" || s.Type == "Arrow Function");
			int classCount    = symbols.Count(s => s.Type == "Class");
			...
			return fileName switch {
				...
				var _ when fileName.EndsWith(".d.ts") => $"TypeScript definitions with {symbols.Count} type declarations.",
				var _                                 => $"{functionCount} functions, {classCount} classes"
			};
		}
		...
		public static async Task GenerateIndexScriptAsync(string agentDir) {
			string scriptPath = Path.Combine(agentDir, "generate-symbol-index.sh");
...
";
		...
		private class SymbolInfo {
			...
			public List<string> Methods     { get; set; } = new List<string>(); // For classes, track their methods
		}
	}
}